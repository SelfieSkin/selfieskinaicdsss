...
const AnatomicalMap = forwardRef<HTMLDivElement, AnatomicalMapProps>(({ 
  ...
  return (
    <div ref={ref} className="relative w-full aspect-[16/9] bg-gray-50 rounded-[3rem] border border-gray-100 overflow-hidden shadow-lg flex items-center justify-center select-none">
      {isGenerating && (
      ...
      {!isGenerating && treatmentMapImageUrl && (
        <>
          ...
          {sites.map(site => (
            ...
            >
              <div className="w-2.5 h-2.5 bg-green-500 rounded-full border border-white shadow-xl group-hover:scale-150 transition-transform"></div>
              <div 
                className="absolute top-0 left-3 whitespace-nowrap text-[9px] font-black text-gray-900" 
            ...
            <div className="no-print absolute top-4 right-4 z-30 flex items-center gap-4">
              <button 
                onClick={() => setShowWireframe(!showWireframe)}
                className={`p-2 rounded-full backdrop-blur-sm transition-all shadow-lg border ${showWireframe ? 'bg-[#cc7e6d] text-white border-white/20' : 'bg-white/60 text-gray-600 border-white/20'}`}
              ...
              <div className="p-2 bg-white/60 backdrop-blur-sm rounded-full flex items-center gap-2 shadow-lg border border-white/20">
                {isGeneratingAnatomy ? (
